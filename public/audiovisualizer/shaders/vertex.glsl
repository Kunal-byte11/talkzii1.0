...
void main() {
    ...
    // Create dynamic vertex displacement
    float displacement = sin(position.x * 2.0 + u_time) * 
                        cos(position.y * 2.0 + u_time) * 
                        sin(position.z * 2.0 + u_time);
    
    // Add frequency-based displacement
    displacement *= u_frequency * 0.5;
