
"use client";

import Image from 'next/image';
import Link from 'next/link'; // Import Link for client-side navigation if needed, or use <a> for anchor
import { Logo } from '@/components/talkzi/Logo'; // Assuming Logo component is preferred

export function NewLandingHeader() {
  return (
    <div className="flex items-center bg-background p-4 pb-2 justify-between">
      <Logo width={80} height={30} /> 
      <a
        href="#footer-contact"
        className="text-primary hover:text-primary/80 font-semibold text-sm px-3 py-2 rounded-md transition-colors"
      >
        Contact Us
      </a>
    </div>
  );
}
